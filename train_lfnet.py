...
def build_training_network(config, next_batch, is_training, psf, global_step):
    ...
    degree_diff1 = tf.reduce_sum(ori_maps1 * ori_maps1w, axis=-1, keepdims=True)
    ...
    degree_diff2 = tf.reduce_sum(ori_maps2 * ori_maps2w, axis=-1, keepdims=True)
    ...
    if config.ori_loss == 'l2loss':
        ...
        ori_loss1 = tf.reduce_sum(ori_maps1 * ori_maps1w, axis=-1, keepdims=True) # both ori_maps have already normalized
        ori_loss1 = tf.reduce_mean( tf.reduce_sum(tf.square(1.0-ori_loss1) * visible_masks1, axis=axis123) / Nvis1)
        ori_loss2 = tf.reduce_mean(ori_maps2 * ori_maps2w, axis=-1, keepdims=True)
    ...
    patches1_canvas = tf.reduce_max(kp_patches1, axis=-1, keepdims=True) # need channel compression in case feat_maps are not photos
    patches1_pos_canvas = tf.reduce_max(kp_patches1_pos, axis=-1, keepdims=True)
    patches1_pos_gt_canvas = tf.reduce_max(kp_patches1_pos_gt, axis=-1, keepdims=True)
    ...
    patches1_canvas = tf.reduce_max(kp_patches1, axis=-1, keepdims=True) # need channel compression in case feat_maps are not photos
    patches1_pos_canvas = tf.reduce_max(kp_patches1_pos, axis=-1, keepdims=True)
    patches1_neg_canvas = tf.reduce_max(kp_patches1_neg, axis=-1, keepdims=True)
