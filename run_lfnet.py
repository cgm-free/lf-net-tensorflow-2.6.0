...
def main(config):
    ...
    # Build Networks
    # tf.reset_default_graph() # tf.reset_default_graph()函数在TensorFlow 2.0及更高版本中已被弃用
    tf.compat.v1.reset_default_graph() # TensorFlow 2.0及更高版本中，您可以使用tf.compat.v1.reset_default_graph()来替换tf.reset_default_graph()。

    photo_ph = tf.placeholder(tf.float32, [1, None, None, 1]) # input grayscale image, normalized by 0~1
    # photo_ph = tf.TensorSpec(shape=(1, None, None, 1), dtype=tf.float32)#  TensorFlow 2.0及更高版本中
